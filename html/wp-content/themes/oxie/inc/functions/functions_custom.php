...
INDEX OF FUNCTIONS
	...
	function mb_get_pinterest_share_url ($post_id) {
	function mb_convert_percentage_to_inverse_ratio ($percentage) {
	function mb_kses_plus ($source, $incl_allowedposttags, $tags = array()) {
	function mb_get_posts_from_show_select ($show, $num_posts, $exclude_current_post) {
	...
	function mb_sanitize_output ($output) {
	function mb_sanitized_output ($output) {
...
***************************************/
	...
	function mb_get_pinterest_share_url ($post_id) {
		...
		// add description
		$excerpt_length = 250;
		$the_excerpt = mb_get_excerpt($post_id, $excerpt_length);
...
/**************************************
mb_convert_percentage_to_inverse_ratio
***************************************/
		...
		elseif (is_page() && !is_front_page()) {
			...
			if($post->post_parent){
				...
				foreach ( $ancestor_ids as $ancestor_id ) {
					$output .= $separator_output;
					$output .= sprintf('<li><a href="%s" title="%s">%s</a></li>', get_permalink($ancestor_id), esc_attr(strip_tags(get_the_title($ancestor_id))), esc_attr(strip_tags(get_the_title($ancestor_id))) );
				}
			}
			$output .= $separator_output;
...
***************************************/
	...
	function mb_sanitized_output ($output) {
		...
		// var_dump($output);
		// var_dump(mb_kses_plus($output, true, array('iframe', 'source')));
		// echo mb_kses_plus($output, true, array('iframe', 'source'));
		
		echo mb_sanitize_output($output);
	}
