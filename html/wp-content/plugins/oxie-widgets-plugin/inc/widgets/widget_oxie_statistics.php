...
/**************************************
WIDGET: oxie_statistics
***************************************/

	add_action('widgets_init', 'register_widget_oxie_statistics' );
	function register_widget_oxie_statistics () {
		register_widget('oxie_statistics');	
	}

	class oxie_statistics extends WP_Widget {
		...
		***************************************/
		function __construct () {
				...
				$widget_ops = array(
					'classname' => 'oxie_statistics', 								
					'description' => __('Display statistics', "loc_oxie_widgets_plugin")	 				
				);
				$control_ops = array(
					'width' => 550, 
					'height' => 350, 
					'id_base' => 'oxie_statistics' 														
				);

				$this->WP_Widget('oxie_statistics', __('Oxie: Statistics', "loc_oxie_widgets_plugin"), $widget_ops, $control_ops );	
		}
		...
		***************************************/
		function form($instance) {
			...
			//defaults
			$defaults = array( 
				'title' 		=> __('Statistics', "loc_oxie_widgets_plugin"),
				'statistic'		=> array(
					0 				=> array(
						'icon'			=> 'fa-flag',
			...
			?>
				...
				<div class="ul_control" data-min="1" data-max="1000">
					...
					<input type="button" class="button ul_del" value="<?php _e("Delete", "loc_oxie_widgets_plugin"); ?>" />
				</div>

				<br>

				<p>
					<label for='<?php echo esc_attr($this->get_field_id('text')); ?>'><?php _e("Statistics text", "loc_oxie_widgets_plugin"); ?></label><br>
				...
				<p>
					<label for="<?php echo esc_attr($this->get_field_id('read_more_link')); ?> "><?php _e("Read More Link (Optional)", "loc_oxie_widgets_plugin"); ?>:</label><br> 
		...
		***************************************/
		function widget($args, $instance) {
			...
			if (function_exists('icl_translate') && function_exists('icl_register_string')) {
				...
				// VERSION > v3.3
				do_action('wpml_register_single_string', 'loc_oxie_widgets_plugin', "$widget_id-widget[text]", $text);
				...
				<?php 
					...
					for ($i = 0; $i < count($statistic); $i++) {  
						...
						if (function_exists('icl_translate') && function_exists('icl_register_string')) {
				            ...
				            // VERSION < 3.3
				            icl_register_string ('loc_oxie_widgets_plugin', "$widget_id-widget[$i][stat_text]", $statistic[$i]['stat_text']);
				            icl_register_string ('loc_oxie_widgets_plugin', "$widget_id-widget[$i][stat_num]", $statistic[$i]['stat_num']);

				            $statistic[$i]['stat_text'] = icl_translate('loc_oxie_widgets_plugin', "$widget_id-widget[$i][stat_text]", $statistic[$i]['stat_text']);
				            $statistic[$i]['stat_num'] = icl_translate('loc_oxie_widgets_plugin', "$widget_id-widget[$i][stat_num]", $statistic[$i]['stat_num']);
						
						} elseif (class_exists('SitePress')) {
				            
				            // VERSION > v3.3
				            do_action('wpml_register_single_string', 'loc_oxie_widgets_plugin', "$widget_id-widget[$i][stat_text]", $statistic[$i]['stat_text']);
				            do_action('wpml_register_single_string', 'loc_oxie_widgets_plugin', "$widget_id-widget[$i][stat_num]", $statistic[$i]['stat_num']);
				            
				            $statistic[$i]['stat_text'] = apply_filters('wpml_translate_single_string', $statistic[$i]['stat_text'], 'loc_oxie_widgets_plugin', "$widget_id-widget[$i][stat_text]");
				            $statistic[$i]['stat_num'] = apply_filters('wpml_translate_single_string', $statistic[$i]['stat_num'], 'loc_oxie_widgets_plugin', "$widget_id-widget[$i][stat_num]");
